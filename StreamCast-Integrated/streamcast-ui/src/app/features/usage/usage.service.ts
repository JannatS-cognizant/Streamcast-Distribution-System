...
import { ScheduleApiResponse } from '../../core/models/schedule';
...
export class UsageService {
  ...
  summary(start: string, end: string): Observable<UsageSummary> {
    ...
    return this.http.get<ScheduleApiResponse<UsageSummary>>(`${this.base}/summary`, { params })
      .pipe(map(r => r.data));
  }
  details(start: string, end: string): Observable<UsageDetails[]> {
    const params = new HttpParams().set('start', start).set('end', end);
    return this.http.get<ScheduleApiResponse<UsageDetails[]>>(`${this.base}/details`, { params })
      .pipe(map(r => r.data));
  }
  breakdown(start: string, end: string, groupBy: string): Observable<UsageBreakdown[]> {
    ...
    return this.http.get<ScheduleApiResponse<UsageBreakdown[]>>(`${this.base}/breakdown`, { params })
      .pipe(map(r => r.data));
  }
}
