...
const STORAGE_LAST_SEEN = 'sc.notifications.lastSeen';      // { contract: number, schedule: number }
const STORAGE_DISMISSED = 'sc.notifications.dismissed';     // string[] of notification ids
...
export class NotificationsService {
  ...
  constructor() {
    // Start polling when authenticated; stop and clear when not.
    effect(() => {
  ...
  private mergeContracts(list: Contract[]): void {
    ...
    const valid = list.filter(c => typeof c.contractId === 'number') as (Contract & { contractId: number })[];
    const maxId = valid.reduce((m, c) => Math.max(m, c.contractId), 0);
    if (last.contract === 0) {
      // First boot — treat everything as already known so we don't fire a flood.
      this.writeLastSeen({ ...last, contract: maxId });
      return;
    }
    const fresh = valid
  ...
  private mergeSchedules(list: CalendarSchedule[]): void {
    ...
    if (last.schedule === 0) {
      this.writeLastSeen({ ...last, schedule: maxId });
      return;
    }
    const fresh = list
  ...
  clearAll(): void {
    ...
    const dismissed = this.readDismissed();
    localStorage.setItem(STORAGE_DISMISSED, JSON.stringify([...dismissed, ...ids].slice(-200)));
    this._items.set([]);
  }
